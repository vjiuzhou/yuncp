...
//		}
	...
	{
		if ($columns === '*' || $columns==='count(*)' || $columns==='count(1)')
		{
			return $columns;
		}
     ...
     * @return array|bool
     */
	public function select($table, $columns = '*',$where = [],$page=1,$pagesize=10,$orderby=[],$join=null)
	{
		$map = [];
		$stack = [];
		$column_map = [];
        ...
        $where=array_merge($where,['LIMIT'=>[($page-1)*$pagesize,$pagesize]],$orderby);

//        echo $this->selectContext($table, $map, $join, $columns, $where);exit;
		$query = $this->exec($this->selectContext($table, $map, $join, $columns, $where), $map);

		//$this->columnap($columns, $column_map);
        if (!$query)
        {
            return false;
        }
	...
	public function insert($table, $datas)
	{
      ...
      //  echo 'INSERT INTO ' . $this->tableQuote($table) . ' (' . implode(', ', $fields) . ') VALUES ' . implode(', ', $stack); exit;
		$result=$this->exec('INSERT INTO ' . $this->tableQuote($table) . ' (' . implode(', ', $fields) . ') VALUES ' . implode(', ', $stack), $map);
		if($result){
	...
	//	echo 'UPDATE ' . $this->tableQuote($table) . ' SET ' . implode(', ', $fields) . $this->whereClause($where, $map);exit;
     ...
     */
	public function find($table, $columns = "*", $where = null,$join = null)
	{
		$map = [];
		$stack = [];
		$column_map = [];

		//$column = $where === null ? $columns : $join;
        $column=$columns;
		$is_single_column = (is_string($column) && $column !== '*');
		//echo $this->selectContext($table, $map, $join, $columns, $where) . ' LIMIT 1';exit;
	...
	public function has($table, $where = null,$join=null)
	{
		$map = [];
		$column = null;
	...
	public function count($table,  $where = null,$join = null, $column = "count(1)")
	{
		$map = [];
     //   echo $this->selectContext($table, $map, $join, $column, $where, 'COUNT');
		$query = $this->exec($this->selectContext($table, $map, $join, $column, $where, 'COUNT'), $map);

		return $query ? 0 + $query->fetchColumn() : false;
	}

	public function max($table,  $column = null, $where = null,$join)
	{
		$map = [];
	...
	public function min($table,  $column = null, $where = null,$join)
	{
		$map = [];
	...
	public function avg($table, $column = null, $where = null,$join)
	{
		$map = [];
	...
	public function sum($table,  $column = null, $where = null,$join)
	{
		$map = [];
