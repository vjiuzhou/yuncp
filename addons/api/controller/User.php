...
class User extends UserModel
{
    ...
    public function addUserFromWannengcha()
    {
        $validate = new UserValidate();
        $datas = $validate->addUserFromWannengchaValidate($this->post);
        /**
         * 判断验证是否有报错信息
         */
        ...
        if ($result) {
            ...
            if ($datas) {
                ...
                $token=$tokenServices->createToken($datas);
                $datas['token']=$token;
                return $this->returnMessage(1001, '响应成功', $datas);
            }
    ...
    public function getUserInfoByuuid()
    {
        ...
        $datas = $validate->findUserByuuid($this->get);
        ...
        /**验证用户是否存在**/
        $user = $this->userHasByuuid($datas['uuid']);
        if ($user == false) {
            return $this->returnMessage(2001, '响应错误', '该用户数据库不存在这条记录');
        }
        $result = $this->getUser(['uuid' => $datas['uuid']]);
        ...
        if ($result) {
             ...
             */
            $tokenServices=new TokenServices();
            $token=$tokenServices->createToken($result);
            $result['token']=$token;
            return $this->returnMessage(1001, '响应成功', $result);
        }
        return $this->returnMessage(2001, '响应错误', []);
     ...
     * @return array
     */
    public function userHasByuuid($uuid)
    {
        /**验证用户是否存在**/
        $user = $this->getUser(['uuid' => $uuid]);
