...
use Addons\api\model\ArticleModel;
use Addons\api\validate\ArticleValidate;
...
class Article extends ArticleModel
{
    ...
    public function selectArea(){
        ...
        $this->post['page_size']=1000;
        $result = $this->selectAreaDB($this->post);
        if ($result) {
    ...
    public function selectManufacturer(){
        $this->post['current_page']=1;
        $this->post['page_size']=1000;
        $result = $this->selectManufacturerDB($this->post);
        if ($result) {
            return $this->returnMessage(1001, '响应成功', $result);
        } else {
            return $this->returnMessage(2001, '响应错误', $result);
    ...
    public function addPost()
    {
         ...
         * 初始化数据
         */
        $validateResult['created_time']  = time();
        $validateResult['updated_time']  = time();
        $validateResultParameter['created_time']  = time();
        $validateResultParameter['updated_time']  = time();
        ...
        if ($result) {
            $validateResultParameter['article_id']=$result;
            $resultParameter = $this->insertParameter($validateResultParameter);
    ...
    public function createTask()
    {
        ...
        $validateResult = $validate->createTaskValidate();
         ...
         * 初始化数据
         */
        $TaskData['created_time']  = time();
        $TaskData['updated_time']  = time();
        $result = $this->findPost($validateResult['id']);
        $TaskData['manufacturer_id']=$result['manufacturer_id'];
        $TaskData['host']=$result['host'];
        $TaskData['article_id']=$result['article_id'];
        $TaskData['task_num']=0;
        $TaskData['status']=0;
         ...
         * 插入数据到数据库
         */
        $result = $this->insertTask($TaskData);
    ...
    public function changePost()
    {
        ...
        if ( !$result) {
            return $this->returnMessage(2001, '响应错误', ['id' => '该文章不存在']);
        }
    ...
    public function delPost()
    {
        ...
        if ( !$result) {
            ...
            if(!empty($validateResult['status'])){
                $data = ['status' => (int)$validateResult['status'], 'updated_time' => time()];
            }else{
                $data = ['status' => 3, 'updated_time' => time()];
 ...
 */
    public function getPostList()
    {
        ...
        $validate       = new ArticleValidate($get);
        ...
        if ($count > 0) {
            $column = 'p.*,u.username, u.avatar, c.name as cname';
     ...
     * @param int    $status       状态：0未解决，1已解决，2精华
     */
    public function getWncPostList()
    {


        $validate       = new ArticleValidate($this->get);
        $validateResult = $validate->getWncPostListValidate();
        ...
        $bulletin= $this->selectBulletin();

        /**
         * 用户热议
         */

        $userHost = $this->selectUserHot();


        return $this->returnMessage(1001, '响应成功', ['bulletin'=>$bulletin,'userhot'=>$userHost]);
    }
    ...
    public function getPostListByAdmin()
    {
        ...
        $validate = new ArticleValidate($post);
     ...
     * @function 后台获取帖子列表
     */
    public function getPostListByRanking()
    {
        $post = $this->getDefaultPage($this->post);

        $validate = new ArticleValidate($post);

        $validateResult = $validate->getPostListByRankingValidate();
        /**
         * 判断验证是否有报错信息
         */
        if (@$validateResult->code == 2001) {
       ...
       // p($validateResult);
         ...
         * 获取当前页数总条数
         */
        $count = $this->countRanking($validateResult);
        ...
        if ($count > 0) {
            ...
            $result = $this->selectPostByranking($validateResult, $column);
    ...
    public function getPost()
    {
      ...
      //  $resultParameter = $this->findParameter($result['id']);
