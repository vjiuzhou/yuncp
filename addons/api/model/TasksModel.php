...
class TasksModel extends BaseModel
{
    ...
    public function selectAreaDB($data){
        $result=db_select(self::$areaTableName,$fields = "*",$where = [],$data['current_page'], $data['page_size']);
 ...
 */
    ...
    {
        return db_update(self::$taskTableName, $data, $where);
    }
    ...
    public function selectPostByAdmin($data, $column = '*')
    {
             ...
             $sql = "SELECT p.*,a.title FROM " . table_prefix(self::$taskTableName) .
                 ' AS p LEFT JOIN ' . table_prefix(self::$tableName) . " AS a ON p.article_id = a.id " ;
    ...
    public function countPost($data)
    {
       ...
       // $sql = 'SELECT  count(*)  AS num FROM ' . table_prefix(self::$taskTableName);
...
//        }
     ...
     * @return bool
     */
    public function findParameter($id, $column = ['memory','cpu','ssd','bandwidth','ipv4','ipv6','outflow','month_price','half_year_price','year_price'])
    {
