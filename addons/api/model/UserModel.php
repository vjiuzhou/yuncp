...
class UserModel extends BaseModel {
    ...
    private static $wncTableName = 'user';

    private static $replieTableName = 'replies';

    private static $postTableName = 'posts';
    private static $MembershipTableName = 'membership';
    ...
    {
        $data= db_find(self::$tableName, $fields, $where);
        if(!empty($data['uuid'])){
            //显示vip
            $sql="select m.membership_grade from ".self::$wncTableName."  as u LEFT JOIN ".self::$MembershipTableName." as m  ON u.uid=m.uid   where u.uuid ='{$data['uuid']}'";
            $mdata=db_fetch($sql);
            if(!empty($mdata['membership_grade'])){
                $data['membership_grade']=$mdata['membership_grade'];
            }
            return $data;
        }
        return false;
